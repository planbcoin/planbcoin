...
###   planbcoincore.org and planbcoin.org and compares them.
...
WORKINGDIR="/tmp/planbcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://planbcoincore.org"
HOST2="https://planbcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="planbcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'planbcoin-',
...
if [ $? -ne 0 ]; then
   echo "planbcoin.org failed to provide signature file, but planbcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "planbcoin.org and planbcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Planbcoin Core binary release signing key installed?"
   fi
