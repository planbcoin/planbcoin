#!/bin/bash
# Copyright (c) 2013 The Planbcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Planbcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Planbcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Planbcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Planbcoin 0.7 datadir."
    ;;
  3)
    echo "Detected Planbcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Planbcoin 0.8 datadir."
    ;;
esac
