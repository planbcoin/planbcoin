...
The RPM spec file provided here is for Planbcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://planbcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Planbcoin git repository](https://github.com/planbcoin/planbcoin) but are not part of
...
Currently only `Source10` is used, for the example `planbcoin.conf` file.
...
in the planbcoin git repository. At present only an SVG version of the Planbcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the planbcoin
...
needed when 0.12.1 is released, a proper fix is already in the Planbcoin git
master branch.
...
idea to build Planbcoin Core against a static version of that library compiled
...
The `planbcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `planbcoin-server` package. At this point, most RPM based
...
The `planbcoin-server` package should have SELinux support. How to properly do
...
Planbcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/planbcoin/). He has been
packaging Planbcoin for Fedora at least since 2012.
...
4. I renamed the `planbcoin` package that contains the Qt GUI to `planbcoin-core` as
that appears to be how the general population refers to it, in contrast to
`planbcoin-xt` or `planbcoin-classic`. I wanted to make sure the general population
