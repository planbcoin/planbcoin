...
#if defined(NDEBUG)
# error "Planbcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Planbcoin Signed Message:\n";
...
void ThreadScriptCheck() {
    RenameThread("planbcoin-scriptch");
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    bool fCacheLarge = mode == FLUSH_STATE_PERIODIC && cacheSize > std::max((9 * nTotalSpace) / 10, nTotalSpace - MAX_BLOCK_COINSDB_PLBGE * 1024 * 1024);
