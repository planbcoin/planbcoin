...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CPlanbcoinAddress(CScriptID(redeemScript)).ToString();
...
std::map<CTxDestination, std::vector<COutput>> CWallet::ListCoins() const
{
    ...
    // https://github.com/planbcoin/planbcoin/pull/10340, but that change has been
    // postponed until after https://github.com/planbcoin/planbcoin/pull/10244 to
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-planbcoin-address
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!strPurpose.empty() && !CWalletDB(*dbw).WritePurpose(CPlanbcoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(*dbw).WriteName(CPlanbcoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        std::string strAddress = CPlanbcoinAddress(address).ToString();
    ...
    CWalletDB(*dbw).ErasePurpose(CPlanbcoinAddress(address).ToString());
    return CWalletDB(*dbw).EraseName(CPlanbcoinAddress(address).ToString());
}
 ...
 * https://planbcointalk.org/?topic=54527, or
 * https://github.com/planbcoin/planbcoin/pull/1393.
 */
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    return CWalletDB(*dbw).WriteDestData(CPlanbcoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    if (!mapAddressBook[dest].destdata.erase(key))
        return false;
    return CWalletDB(*dbw).EraseDestData(CPlanbcoinAddress(dest).ToString(), key);
}
