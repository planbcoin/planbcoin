...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s planbcoin-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  planbcoin-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded planbcoin transaction") + "\n" +
              "  planbcoin-tx [options] -create [commands]   " + _("Create hex-encoded planbcoin transaction") + "\n" +
              "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CPlanbcoinAddress addr(strAddr);
    if (!addr.IsValid())
...
static void MutateTxAddOutPubKey(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (bScriptHash) {
        ...
        CPlanbcoinAddress redeemScriptAddr(scriptPubKey);
...
static void MutateTxAddOutMultiSig(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    if (bScriptHash) {
        // Get the address for the redeem script, then call
        // GetScriptForDestination() to construct a P2SH scriptPubKey.
        CPlanbcoinAddress addr(scriptPubKey);
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded planbcoin transaction
