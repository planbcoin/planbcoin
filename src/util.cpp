...
const char * const BITCOIN_CONF_FILENAME = "planbcoin.conf";
const char * const BITCOIN_PID_FILENAME = "planbcoind.pid";
...
#else
    const char* pszModule = "planbcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Planbcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Planbcoin
    // Mac: ~/Library/Application Support/Planbcoin
    // Unix: ~/.planbcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Planbcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Planbcoin";
#else
    // Unix
    return pathRet / ".planbcoin";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good())
        return; // No planbcoin.conf file is OK

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override planbcoin.conf
