    ...
    signalled(false)
{
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the Planbcoin block chain.").arg(tr(PACKAGE_NAME)) + " " +
...
bool Intro::pickDataDirectory()
{
    ...
    if(!fs::exists(GUIUtil::qstringToBoostPath(dataDir)) || GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || settings.value("fReset", false).toBool() || GetBoolArg("-resetguisettings", false))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/planbcoin"));
     ...
     * override -datadir in the planbcoin.conf file in the default data directory
     * (to be consistent with planbcoind behavior)
     */
    if(dataDir != getDefaultDataDirectory())
