...
*Setup instructions for a Gitian build of Planbcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Planbcoin
...
- [Building Planbcoin Core](#building-planbcoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-planbcoin-host/README.md).
...
Clone the git repositories for planbcoin and Gitian.
...
in the planbcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Planbcoin Core
----------------

To build Planbcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the planbcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/planbcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/planbcoin/planbcoin
...
For example:
```bash
URL=https://github.com/laanwj/planbcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit planbcoin=${COMMIT} --url planbcoin=${URL} ../planbcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit planbcoin=${COMMIT} --url planbcoin=${URL} ../planbcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit planbcoin=${COMMIT} --url planbcoin=${URL} ../planbcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the planbcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../planbcoin/contrib/gitian-descriptors/*|sort|uniq )
...
PLBPATH=/some/root/path/planbcoin
SIGPATH=/some/root/path/planbcoin-detached-sigs

./bin/gbuild --url planbcoin=${PLBPATH},signature=${SIGPATH} ../planbcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/planbcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/planbcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/planbcoin-osx-build.assert
```
...
[planbcoin-core/gitian.sigs](https://github.com/planbcoin-core/gitian.sigs/) repository, or if that's not possible create a pull
