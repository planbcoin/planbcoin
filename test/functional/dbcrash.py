#!/usr/bin/env python3
# Copyright (c) 2017 The Planbcoin Core developers
...
class ChainstateWriteCrashTest(PlanbcoinTestFramework):
    def __init__(self):
        super().__init__()
        self.num_nodes = 4
    ...
    def restart_node(self, node_index, expected_tip):
        ...
        while time.time() - time_start < 60:
            try:
                ...
                # If planbcoind exits, then try again.  wait_for_node_exit()
                # should raise an exception if planbcoind doesn't exit.
                self.wait_for_node_exit(node_index, timeout=10)
            self.crashed_on_restart += 1
            time.sleep(1)

        # If we got here, planbcoind isn't coming back up on restart.  Could be a
        # bug in planbcoind, or we've gotten unlucky with our dbcrash ratio --
